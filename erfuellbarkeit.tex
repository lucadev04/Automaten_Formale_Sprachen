...
\begin{frame}{Erfüllbarkeitstest (Markierungsalgorithmus)}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			\item[(i)] $A_1 \land A_2 \land \ldots \land A_n \rightarrow B$ bzw. $w \rightarrow B$ oder
...
\note{\scriptsize
	\begin{align*}
		F &= (\neg A \lor \neg B \lor \neg D) \land \neg E \land (\neg C \lor A) \land C \land B \land (\neg G \lor D) \land G\\
		&= (A \land B \land D \rightarrow f) \land (E \rightarrow f) \land (C \rightarrow A) \land \\
		& \qquad (w \rightarrow C) \land (w \rightarrow B) \land (G \rightarrow D) \land (w \rightarrow G) \\
		&= (A \land \stkout{B} \land D \rightarrow f) \land (E \rightarrow f) \land (\stkout{C} \rightarrow A) \land \\
		& \qquad \stkout{(w \rightarrow C)} \land \stkout{(w \rightarrow B)} \land (\stkout{G} \rightarrow D) \land \stkout{(w \rightarrow G)} \\
		&= (\stkout{A} \land \stkout{B} \land \stkout{D} \rightarrow f) \land (E \rightarrow f) \land \stkout{(\stkout{C} \rightarrow A)} \land \\
		& \qquad \stkout{(w \rightarrow C)} \land \stkout{(w \rightarrow B)} \land \stkout{(\stkout{G} \rightarrow D)} \land \stkout{(w \rightarrow G)}\\
		& \textrm{unerfüllbar, wegen } (\stkout{A} \land \stkout{B} \land \stkout{D} \rightarrow f)
	\end{align*}
}
\begin{frame}{Erfüllbarkeitstest (Hornklauselalgorithmus)}
	...
	\qquad entferne alle Klauseln, die $A_i$ enthalten\\
	\qquad und entferne $\overline{A_i}$ aus den verbliebenen Klauseln\\
	end\\
	if ($S$ enthält die leere Klausel $\Box$)\\
...
\begin{frame}{Resolution}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item dann heißt die Klausel $RES_L(K_1, K_2)=\left(K_1 \setminus \left\{L\right\}\right) \cup \left(K_2 \setminus \left\{\overline{L}\right\}\right)$,\\ gesprochen: \emph{Resolvente} der Klauseln $K_1$ und $K_2$ nach $L$
			\item Notation: $\left\{K_1, K_2\right\} \vdash_{res} RES_L(K_1, K_2)$
		\end{itemize}
...
\begin{frame}{Grundresolutionstheorem}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item beachte: $RES_L(S)$ enthält $L$ bzw. $\overline{L}$ nicht mehr
