...
\begin{frame}{Spezifikation PDA}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item $\delta$: Zustandsüberführungsrelation, $\delta \subseteq S \times \left(\Sigma \cup \left\{\varepsilon\right\}\right) \times \Gamma \times S \times \Gamma^*$
...
\begin{frame}{Zusammenhang mit kontextfreien Sprachen}
	\begin{itemize}
		...
		\item zu jeder kontextfreien Grammatik lässt sich ein (i.Allg. nichtdeterministischer) PDA konstruieren, der die gleiche Sprache akzeptiert
...
\begin{frame}{PDA, der mit leerem Keller akzeptiert $\rightarrow$ kontextfreie Grammatik}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			$[s_j,A,p_m] \rightarrow a[p_0,B_1,p_1][p_1,B_2,p_2]\ldots[p_{m-1},B_m,p_m]$\\
			mit $p_0=s_k$, für alle Kombinationen $p_1,\ldots,p_m \in S$
...
Pac-Man \footnote{Applications of Deterministic Finite Automata, Eric Gribkoff, \url{https://www.cs.ucdavis.edu/~rogaway/classes/120/spring13/eric-dfa.pdf}}
		\begin{columns}
			\column{.4\textwidth}
		\begin{figure}
			\includegraphics[width=0.9\linewidth]{images/Pac-Man}
		\end{figure}
		\column{.6\textwidth}
		\resizebox{\linewidth}{!}{%
		\begin{tikzpicture}
...
\begin{frame}{Exkurs: PDA im RL}
	Drehkreuz
	\begin{center}
		% Zeichnung sitzt oben links auf einer leeren A4-Leinwand -> leeren Bereich wegschneiden
		\includegraphics[trim=0 580bp 177bp 0, clip, width=0.55\linewidth]{images/turnstile}
	\end{center}
\end{frame}