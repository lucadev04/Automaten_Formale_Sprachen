...
\begin{frame}{Durch DFA akzeptierte Sprachen: Reguläre Sprachen}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item[$P:$] für jeden Übergang $\delta(s_i,a)=s_j$ die Regel $S_i \rightarrow aS_j$\\
			zusätzlich die Regel $S_i \rightarrow a$, falls $s_j$ Endzustand\\
			ist $s_0$ Endzustand: neue Startvariable $S'$ mit $S' \rightarrow \varepsilon$ sowie allen Regeln von $S_0$ ($S'$ steht dann auf keiner rechten Seite)
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Simulation des Verhaltens von NFAs}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\quad $\hat{\delta}: \mathcal{P}(S) \times \Sigma^* \rightarrow \mathcal{P}(S)$\\
...
\begin{frame}{Äquivalenz von DFA und NFA}
	\begin{itemize}
		...
		\quad $\delta_d(S', a)=\bigcup_{s \in S'} \delta(s, a) \in \mathcal{P}(S)$\\
...
\begin{frame}{Beispiel: Umwandlung NFA in DFA (Zustandsgraph)}
	\begin{columns}
		...
		\begin{tikzpicture}
			...
			\node[state, initial, initial where=below, below=of s0] (s0s1) {$s_0s_1$};
			\node[state, accepting, right=of s0] (s0s1s2) {$s_0s_1s_2$};
			
			\draw
			(s0) edge[bend left, left] node{0} (s0s1)
			(s0) edge[loop left] node{1} (s0)
			(s0s1) edge[below right] node{0} (s0s1s2)
			(s0s1) edge[bend left, left] node{1} (s0)
			(s0s1s2) edge[loop right] node{0} (s0s1s2)
			(s0s1s2) edge[above] node{1} (s0);
		\end{tikzpicture}
	\end{columns}
...
\begin{frame}{Abgeschlossenheit regulärer Sprachen}
	...
	\begin{itemize}
		\item jede endliche Sprache $L_{3} \subseteq \Sigma^*$
		\item $L = \Sigma^*$
		\item $L_1 \cup L_2$
...
\begin{frame}{Reguläre Ausdrücke}
	\begin{itemize}
		...
		\item Siehe auch: \url{https://stackoverflow.com/questions/1732348}
	\end{itemize}
\end{frame}
